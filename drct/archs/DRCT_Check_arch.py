...
class RDG(nn.Module):
    ...
    def forward(self, x, xsize):
        with record_function("RDG/swin1"):
            s1 = self.swin1(x, xsize)
        with record_function("RDG/pue1"):
            u1 = self.pue(s1, xsize)
        with record_function("RDG/adjust1"):
            a1 = self.adjust1(u1)
        with record_function("RDG/act1+pe1"):
            x1 = self.pe(self.lrelu(a1))

        with record_function("RDG/cat12"):
            c12 = torch.cat((x, x1), -1)
        with record_function("RDG/swin2"):
            s2 = self.swin2(c12, xsize)
        with record_function("RDG/pue2"):
            u2 = self.pue(s2, xsize)
        with record_function("RDG/adjust2"):
            a2 = self.adjust2(u2)
        with record_function("RDG/act2+pe2"):
            x2 = self.pe(self.lrelu(a2))

        with record_function("RDG/cat123"):
            c123 = torch.cat((x, x1, x2), -1)
        with record_function("RDG/swin3"):
            s3 = self.swin3(c123, xsize)
        with record_function("RDG/pue3"):
            u3 = self.pue(s3, xsize)
        with record_function("RDG/adjust3"):
            a3 = self.adjust3(u3)
        with record_function("RDG/act3+pe3"):
            x3 = self.pe(self.lrelu(a3))

        with record_function("RDG/cat1234"):
            c1234 = torch.cat((x, x1, x2, x3), -1)
        with record_function("RDG/swin4"):
            s4 = self.swin4(c1234, xsize)
        with record_function("RDG/pue4"):
            u4 = self.pue(s4, xsize)
        with record_function("RDG/adjust4"):
            a4 = self.adjust4(u4)
        with record_function("RDG/act4+pe4"):
            x4 = self.pe(self.lrelu(a4))

        with record_function("RDG/cat12345"):
            c12345 = torch.cat((x, x1, x2, x3, x4), -1)
        with record_function("RDG/swin5"):
            s5 = self.swin5(c12345, xsize)
        with record_function("RDG/pue5"):
            u5 = self.pue(s5, xsize)
        with record_function("RDG/adjust5+pe5"):
            x5 = self.pe(self.adjust5(u5))

        with record_function("RDG/out"):
            out = x5 * 0.2 + x
        return out
...
class DRCT_Check(nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_size = patch_size
        print(">>> patch_size =", self.patch_size, "img_size =", img_size, "patches_resolution =", self.patches_resolution)
    ...
    def forward_features(self, x):
        ...
        for i, layer in enumerate(self.layers):
            with record_function(f"DRCT/RDG_{i+1}"):
                x = layer(x, x_size)

        if torch.jit.is_tracing() and x.dim() == 3 and x.shape[-1] != self.num_features and x.shape[1] == self.num_features:
            x = x.transpose(1, 2)
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            with record_function("DRCT/conv_first"):
                x = self.conv_first(x)

            with record_function("DRCT/body"):
                x = self.conv_after_body(self.forward_features(x)) + x

            with record_function("DRCT/head"):
