...
class KDDRCT(nn.Module):
    ...
    def __init__(self,
        ...
        # ✅ (핵심 수정) mean subtraction 끔: PSNR 12dB 문제의 1순위 원인 제거
        self.img_range = 1.0
    ...
    def forward_features(self, x, return_feat=False):
        ...
        for layer in self.layers:
            ...
            if return_feat:
                # (B, HW, C) -> (B, C, H, W)
                feats.append(self.patch_unembed(self.norm(x), x_size))
    ...
    def forward(self, x, return_feat=False):
        ...
        # ✅ window_size 배수 pad (val/test에서 크기 안 맞으면 crash나는 문제 해결)
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            if return_feat:
                body, feats = self.forward_features(x, return_feat=True)
                x = self.conv_after_body(body) + x
            else:
                x = self.conv_after_body(self.forward_features(x, return_feat=False)) + x
        ...
        # ✅ pad 했으면 crop (업스케일 반영)
        if pad_h != 0 or pad_w != 0:
            scale = self.upscale
            x = x[:, :, :H * scale, :W * scale]
            if return_feat:
                feats = [f[:, :, :H, :W] for f in feats]
